...
# The comparision will be how nutrients (NPK) content will differ according to tissue and position of sampling 
...
fig1
fig2<- fig1+ stat_summary(fun.y=mean,# To add bar plot of means
                          geom="bar",
                          aes(fill=tissue))
fig2
fig3<- fig2+stat_summary(fun.data=mean_se, # To add SE on bar
                         geom="errorbar",
                         width=.2)
fig3
...
fig5<- fig4+theme(plot.title= element_text(hjust = 0.5, face="bold", colour= "blue"))
fig5
fig6<- fig5+theme(axis.title=element_text(face="bold",size=rel(1.5)))
fig6
fig7<- fig6+ theme(axis.line= element_line(colour="black", size=rel(2)))
fig7
fig8<- fig7+theme(axis.text= element_text(face="bold", colour='blue', angle=90))
fig8

###### CUSTOMIZATION OF PANELS--------------------------------
fig9<- fig8+theme(panel.background = element_rect(fill= "grey"))
fig9
fig10<- fig9+ theme(panel.spacing.x = unit(0.5, "lines"))
fig10
fig11<- fig10 + theme(strip.background=element_rect(fill="green"))
fig11               
fig12<- fig11+ theme(strip.text=element_text(face='bold', family='Times New Roman', colour='blue'))
fig12
fig13<- fig12+theme(strip.text.y=element_text(angle=0))
fig13
